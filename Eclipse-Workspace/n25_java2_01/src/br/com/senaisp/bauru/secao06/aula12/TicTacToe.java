...
public class TicTacToe {
	...
	public void lancarJogada(int id) {
		...
		if(ehFimJogo()) vencedor = "Empate" ;
		if (posDisp.length>0 && ehFimJogo()) vencedor = getNomePlayer();
		if (posDisp.length>0 && !ehFimJogo()) {
			int idx = rnd.nextInt(posDisp.length);
			tab[posDisp[idx]] = 'X';
			atualizarPosLivre(posDisp[idx]);
			if (posDisp.length>0 && ehFimJogo()) vencedor = "computador";
			if(posDisp.length==0 && ehFimJogo()) { vencedor = "Empate";
